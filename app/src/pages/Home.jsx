...
function Home() {
  ...
  return <Container className="sm:1/2 font-['Poppins'] h-full">
    {user.role === 'nanny' && <><header className='flex flex-row justify-between items-center fixed top-0 bg-white z-10'>
    <Link to="/">
        <img src="images/kangaroo.png" alt="kangaroo" className="w-14 h-14 mr-2" />
      </Link>
      <h1 className='flex flex-col justify-center text-[#fb923c] mr-2' >NANNY KANGAROO</h1>
      ...
      <footer className="sm: w-full fixed bottom-0 bg-[#fb923c] rounded-md">
        <nav className='flex justify-between'>
          <Link to="/" className="m-3" ><HomeIcon className="h-8 w-8 text-white" /></Link>


          <Link to="/search/parents" className="m-3" ><MagnifyingGlassIcon className="h-8 w-8 text-white" /></Link>

          <Link to="/parents/favs" className=" m-3"><StarIcon className="h-8 w-8 text-white" /></Link>
          <Link to="/chats" className=" m-3"><ChatBubbleLeftRightIcon className='h-8 w-8 text-white' /></Link>
          <button onClick={handleClick} className='mx-3 text-center'>
            {showNav ? <UserCircleIcon className="h-8 w-8 text-white" /> : <Bars3Icon className='h-8 w-8 text-white' />}
          </button>
          <>
            {showNav && <div onClick={handleClick} className="w-full h-full fixed">
              <ul className='flex flex-col   items-center bg-[#fb923c] fixed top-24 text-white text-xl mt-2 py-4  w-52 gap-y-8 rounded right-0'>
    ...
    {user.role === 'parent' && <> <header className='flex flex-row justify-between items-center fixed top-0 bg-white z-10'>
      <Link to="/">
      ...
      <footer className="sm: w-full position fixed bottom-0 bg-[#fb923c] rounded-md">
        <nav className='flex justify-between'>
          ...
          <Link to="/search/nannies" className="m-3" ><MagnifyingGlassIcon className="h-8 w-8 text-white" /></Link>

          <Link to="/nannies/favs" className="m-3" ><StarIcon className="h-8 w-8 text-white" /></Link>
          <Link to="/chats" className=" m-3"><ChatBubbleLeftRightIcon className='h-8 w-8 text-white' /></Link>
          <button onClick={handleClick} className='mx-3 text-center'>
            {showNav ? <UserCircleIcon className="h-8 w-8 text-white" /> : <Bars3Icon className='h-8 w-8 text-white' />}
          </button>
